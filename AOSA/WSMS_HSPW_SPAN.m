function [CRB_theta] = WSMS_HSPW_SPAN(psi0,theta,lambda,r,R,K,M,Nr,d)
...
T_S_theta2=K-K*psi0/2/tan(psi0/2);
...
T_S_r=-K/2/tan(psi0/2)*log(abs((1+sin(psi0/2))/(1-sin(psi0/2))));
...
% 
% DQ=Q11*Q22-Q12^2
%CRB_r=sqrt(1/Q11/(K*M*Nr)/2)
CRB_theta=sqrt(1/Q11/(K*M*Nr)/2);

% sqrt(lambda^2/8/pi^2/(d^2*(M^2-1)/12)/K/M/Nr)
end
